...
def get_MAN(content, relation):
    try:
        ...
        ## 这里是处理模型连姓都没抽出来的情况 
        job_title = ["助理","老师","先生","女士","小姐","主任","经理","医生","主管","总监"]
        job_content = content
        ...
        for item,ph,tel in zip_longest(name,phone_num,filter_phone):
            ...
            if len(item) == 1:
                for title in job_title:
                    if title in job_content:
                        job_index = job_content.find(title)
                        name_job = job_content[job_index-1:job_index+len(title)]
                        job_content = job_content[job_index+1:]
                        item = name_job
                        break
...
def get_man_org(content):
    ...
    if pos > 0:
        ...
        need_content = ""
        # need_content = content[len(content)-500:len(content)]
    ...
    # if pos1 < 0:
    #     pos1 = caigou_content.find('招标人联系方式')      
    # if pos1 < 0:
    #     pos1 = caigou_content.find('招标机构')      
